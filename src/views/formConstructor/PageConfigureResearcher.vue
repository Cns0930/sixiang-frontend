    ...
    <div class="page-configure">
        <div class="op-bar">
           ...
           <!-- <el-button @click="handlePreview">预览页面</el-button>-->
            ...
            <el-button @click="$router.push({path:'/run',query:{itemId, type:'researcher'}})">运行页面</el-button>
            <el-button @click="handleOutput">输出</el-button>
            <el-link :href="`bangban.html#/?itemId=${itemId}&barcode=${barcode}&type=researcher`" target="_blank">超级帮办模拟运行</el-link>barcode<el-input v-model="barcode" style="width:100px"></el-input>
...
import { getStepResearcher, saveStepResearcher, deleteStepResearcher, saveStepBatchResearcher } from "@/api/step/index";
...
export default {
    ...
    methods: {
        ...
        async addStepPage() {
            ...
            let result = await saveStepResearcher(data);
        ...
        async handleDeleteStepPage(step) {
            ...
            let result = await deleteStepResearcher({ stepId: step.id })
            if (!result.success) return;
        ...
        async deleteConfig(scope) {
            ...
            let result = await saveStepResearcher(this.temp_page);
        },
        swapUp(scope) {
        ...
        async loadStep() {
            ...
            let result = await getStepResearcher({ approvalItemId: this.itemId });
            if (!result.success) return;
        ...
        async importDefault(){
            ...
            let result = await saveStepBatchResearcher(params);
            if (!result.success) return;
