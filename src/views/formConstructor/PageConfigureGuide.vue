...
import { getStep, saveStep, deleteStep, getConditionSwitch, setConditionSwitch} from "@/api/step/condition";
...
export default {
    ...
    methods: {
        async initSwitch(){
            let result = await getConditionSwitch({approvalItemId : this.$route.query.itemId, switchType: this.switchType})
            if(result.success){
                this.isActive = result.data;
                console.log(this.isActive)
            }else{
                this.$message.warning('初始化页面失败，请联系管理员')
            }

        },
        async changeSwitch(v){
            let result = await setConditionSwitch({
        ...
        async loadStep() {
            ...
            let result = await getStep({ approvalItemId: this.itemId, switchType: this.switchType });
            if (!result.success) return;
        ...
        async loadAll() {
            let result = await Promise.all([
                getStep({approvalItemId: this.$route.query.itemId, switchType: this.switchType}),
                getFieldAll({ approvalItemId: this.$route.query.itemId }),
                getTemplate({ approvalItemId: this.$route.query.itemId })
            ])
